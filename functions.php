...
function pesan($data)
{
  ...
  $lama = $data["jam_mulai"];
  $mulai = $data["tgl_main"];
  $mulai_waktu = strtotime($mulai); // Mengubah format datetime-local menjadi UNIX timestamp
  $habis_waktu = $mulai_waktu + (intval($lama) * 3600); // Menambahkan waktu dalam jam ke waktu awal
  $habis = date('Y-m-d H:i:s', $habis_waktu); // Mengubah format waktu kembali ke datetime-local

  // Mendapatkan waktu mulai dan waktu habis dalam format jam
  $jam_mulai = date('H', $mulai_waktu);
  $jam_habis = date('H', $habis_waktu);

  // Cek apakah waktu pemesanan berada dalam rentang yang tidak diperbolehkan
  if (($jam_mulai >= 0 && $jam_mulai < 8) || ($jam_habis > 0 && $jam_habis <= 8)) {
    echo "<script>
            alert('Pemesanan tidak diperbolehkan antara jam 12 malam hingga 8 pagi.');
          </script>";
    return false;
  }

  // Cek apakah slot waktu sudah dibooking
  $query = "SELECT * FROM sewa_212279 
              WHERE 212279_id_lapangan = '$idlpg' 
              AND (
                  (212279_jam_mulai <= '$mulai' AND 212279_jam_habis > '$mulai') OR
                  (212279_jam_mulai < '$habis' AND 212279_jam_habis >= '$habis') OR
                  ('$mulai' <= 212279_jam_mulai AND '$habis' >= 212279_jam_habis)
              )";

  $result = mysqli_query($conn, $query);

  // Jika ada hasil, berarti slot sudah dibooking
  if (mysqli_num_rows($result) > 0) {
    echo "<script>
            alert('Slot waktu sudah dibooking, silakan pilih waktu lain!');
          </script>";
    return false;
  }

  // Jika tidak ada booking, lanjutkan proses insert booking
  $harga = $data["harga"];
  $total = intval($lama) * intval($harga);

  mysqli_query($conn, "INSERT INTO sewa_212279 
                         (212279_id_user, 212279_id_lapangan, 212279_lama_sewa, 212279_jam_mulai, 212279_jam_habis, 212279_harga, 212279_total) 
                         VALUES 
                         ('$userid','$idlpg','$lama','$mulai','$habis','$harga','$total')");
